...
const TimelineItem = ({ item, index }: TimelineItemProps) => {
    ...
    return (
        ...
        >
            <span className="absolute flex items-center justify-center w-12 h-12 bg-base-100 rounded-full -left-6 ring-8 ring-neutral-content z-10">
                <Image 
                    className="rounded-full shadow-lg object-cover" 
                    src={`/assets/images/${item.logo}`} 
                    alt={t(`experiences.${item.id}.title`)} 
                    width={48} 
                    height={48} 
                />
            </span>
...
const TimelineDark = ({ items }: { items: any[] }) => {
    return (
